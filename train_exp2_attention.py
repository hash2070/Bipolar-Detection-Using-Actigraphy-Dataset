"""
Experiment 2 - Approach 2b: Attention LSTM
==========================================

HYPERPARAMETER JUSTIFICATION:
- num_epochs: 90 (increased from baseline 50)
  WHY: Attention mechanism is complex. Training longer helps attention weights stabilize and
       the model learn which time steps discriminate bipolar/unipolar.

- batch_size: 16 (same as baseline)
  WHY: Balanced dataset still manageable with batch 16. Attention doesn't require change.

- learning_rate: 5e-4 (REDUCED from baseline 1e-3)
  WHY: Attention + LSTM is complex. Lower learning rate helps stable convergence of attention weights.
       Too high learning rate can cause attention to oscillate.

- weight_decay: 1e-4 (same as baseline)
  WHY: Same regularization as baseline - dataset size unchanged.

- patience: 15 (increased from baseline 10)
  WHY: Attention mechanism = more complex = slower convergence. Extra patience prevents early stopping.
"""
...
from model_variants import AttentionLSTMClassifier
...
class Experiment2AttentionTrainer:
    """Train Attention LSTM with balanced downsampled dataset."""
    ...
    def __init__(self,
        ...
        self.model = AttentionLSTMClassifier(num_classes=2).to(self.device)
        ...
        print(f"Attention LSTM total parameters: {total_params:,}")
    ...
    def train(self, split: dict) -> None:
        ...
        print(f"\nTraining Attention LSTM for {self.num_epochs} epochs with patience={self.patience}...")
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                torch.save(self.model.state_dict(), 'best_model_exp2_attention.pt')
            else:
                patience_counter += 1
        ...
        self.model.load_state_dict(torch.load('best_model_exp2_attention.pt'))
...
def run_experiment_2_attention():
    """Run Experiment 2 with Attention LSTM architecture."""
    print("=" * 70)
    print("EXPERIMENT 2 - APPROACH 2b: Attention LSTM")
    print("=" * 70)
    ...
    trainer = Experiment2AttentionTrainer(num_epochs=90, batch_size=16, learning_rate=5e-4, patience=15)
    trainer.train(split)

    results, y_true, y_pred, y_probs = trainer.evaluate(split)

    print("\n" + "=" * 70)
    print("EXPERIMENT 2 - APPROACH 2b RESULTS: Attention LSTM")
    print("=" * 70)
    ...
    with open("results/exp2_approach2b_attention_results.json", "w") as f:
        json.dump({k: v for k, v in results.items() if k != 'confusion_matrix'}, f, indent=2)

    np.save("results/exp2_approach2b_attention_y_true.npy", y_true)
    np.save("results/exp2_approach2b_attention_y_pred.npy", y_pred)
    np.save("results/exp2_approach2b_attention_y_probs.npy", y_probs)

    print("\nResults saved to results/exp2_approach2b_attention_*")
    return results, y_true, y_pred, y_probs


if __name__ == "__main__":
    run_experiment_2_attention()
